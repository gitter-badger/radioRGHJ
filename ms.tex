...
Thus, by extrapolating the radiometric Bode's law, we provide more optimistic estimates for highly evolved stars compared to previous results.
...
\end{eqnarray}
\memoTM{Some kind of plot of the typical ranges of these quantities might be useful/illustrative.}
%%%
...
\end{equation}
%%%
Here, $T_{\rm sys}$ is the blackbody temperature equivalent of the total system noise, which is the sum of the instrument or receiver noise temperature $T_{\rm rx}$ and the noise contribution from the sky $T_{\rm sky}$.
...
\end{equation}
%%%
where $T_{150} = 283.2~\rm K$ is the $T_{\rm sky}$ at 150~MHz, $\beta=2.47$, and $T_{\rm CMB} = 2.73~\rm K$ is the contribution from the cosmic microwave background (CMB).
...
Given the strong frequency dependence of the effective aperture of a dipole antenna, and that the system noise temperature $T_{\rm sys}$ for LOFAR is not fully determined by the sky noise $T_{\rm sky}$ (i.e.\ $T_{\rm rx} \sim T_{\rm sky}$), we use the LOFAR Image Noise calculator\footnote{Heald; \url{http://www.astron.nl/~heald/test/sens.php}}, which is based on {\it SKA Memo 113}\footnote{Nijboer, Pandey-Pommier, \& de Bruyn; \url{http://www.skatelescope.org/uploaded/59513\_113\_Memo\_Nijboer.pdf}}, for our fiducial sensitivity and imaging parameters in Table~\ref{tab:sens}.
...
We adopt the \cite{Rogers+Bowman2008} relation for $T_{\rm sys}$ as above, and assume $T_{\rm rx}\approx 60~\rm K$.
We caution that these assumptions are optimistic, and the time estimates could in reality be a factor of a few worse.

% VLA
For the low frequency receivers (28-80~MHz; 4-Band) in the \memoDS{Long Wavelength Array (LWA) and the?} Very Large Array (VLA), the receiver noise temperature $T_{\rm rx}\approx 260~\rm K$ \citep{Hicks2012} and is subdominant to $T_{\rm sky}$ by at least -6~dB.
...
We estimate the sensitivity of LOBO assuming the receiver noise extrapolates linearly from $T_{\rm rx}\approx 60~\rm K$ (internal VLA memo) in P-Band (230-470~MHz) to $T_{\rm rx}\approx 260~\rm K$ in 4-band, and consider just the 50-350~MHz portion of the full 30-470~MHz band.
