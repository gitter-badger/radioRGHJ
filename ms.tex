...
\begin{abstract}
%%%
Magnetized planets emit radio waves in the interaction with stellar winds. 
...
It has been known that Jupiter emits radio emission from the auroral region due to cyclotron-maser instability \citep{zarka1998}.  % due to the acceleration of the plasmas. \memoYF{?}.
...
While the solar mass loss rate, $\dot M_\odot$, is $\dot M_\odot \sim 2\times 10^{-14} M_{\odot}$/yr \citep[e.g.,][]{hundhausen1997}, the mass loss rate of red giants is typically $\dot M_\star \sim 10^{-8}-10^{-7} M_{\odot}$/yr, and the rate can be as high as $10^{-5} M_\odot{\rm /yr}$ during the AGB phase. 
Therefore, we have
%%%%%%%%%% 
\begin{equation}
...
Assuming that the stellar wind scales as the escape velocity, i.e., $\propto \sqrt{2GM_{\star}/R_{\star}}$ ($G$ is the gravitational constant, $M_{\star}$ is the stellar mass, and $R_{\star}$ is the stellar radius), a RG with radius $R_{\star}=100R_{\odot}$ has 10 times as slow stellar wind as that at the main sequence. 
...
Several relationships have been proposed so far \citep[e.g.][]{russel1978,busse1976,stevenson1979,mizutani1992,sano1993,starchenko2002,christensen2006,christensen_et_al2009} %\memoYF{check!}
%(see \citet{griesmeier2004} or \citet{christensen2010} for the summary), 
which are compared with numerical simulations in \citet{christensen2010}. 
We employ the relationship proposed in \citet{christensen_et_al2009}, which is based on the assumption that the ohmic dissipation energy is a fraction of available convected energy and was found to be in a good agreement with the numerical experiments over a wide parameter space: 
...
%\end{equation}
\begin{equation}
\langle B_c \rangle ^2 \propto f_{\rm ohm} \langle \rho_c \rangle ^{1/3} (Fq_o)^{2/3} \label{eq:Bscaling} 
\end{equation}
%%%%%%%%%%
...
where $\langle B_c \rangle$ is the mean magnetic field in the dynamo region, $f_{\rm ohm}$ is the ratio of ohmic dissipation to the total dissipation, $\langle \rho_c \rangle$ is the mean density in the dynamo region, $F$ is the efficiency factor of order unity, and $q_o$ is the convected flux at the outer boundary of the dynamo region \citep[see][for the comprehensive description]{christensen_et_al2009}. 
Dipole magnetic field strength at the planetary surface, denoted by $B$, is then scaled by
%%%%%%%%%%
\begin{equation}
B \propto B_c \left( \frac{r_c}{R_p} \right)^3 \label{eq:Bscaling2}
\end{equation}
%%%%%%%%%%
Magnetic field of planets are estimated by scaling Jovian magnetic field $B_J \sim 10$~G with equations (\ref{eq:Bscaling}) and (\ref{eq:Bscaling2}) assuming that $f_{\rm ohm}$ and $F$ are roughly constant. 

In order to evaluate $\rho _c $ and $q_o$, we need a model of internal structure of planets. 
...
The scaling of convected heat flux at the outer boundary, $q_o$, is obtained by dividing the time-dependent net planetary luminosity by the surface area of the core region, i.e., $4\pi r_c^2$. 
The model of luminosity evolution is taken from \citet{burrows_et_al2001} \citep[see also][]{marley2007}, which resulted in:
%%%%%%%%%%
\begin{equation}
L \sim 1.4\times 10^{-9} L_\odot \left( \frac{t}{4.5 \rm~Gyr} \right)^{-1.3} \left( \frac{M_p}{M_J} \right)^{2.64} \, 	
\label{eq:burrowsLum}
\end{equation}
%%%%%%%%%%
Therefore, we have
...
Figure \ref{fig:planetaryB} shows the estimated core radius ($r_c$), core density ($\rho_c$), and convected heat flux ($q_o$) as well as the predicted magnetic field strength at planetary surface ($B$) and the corresponding cyclotron frequency ($f_{\rm cyc}$), as functions of planetary mass and the age. 
