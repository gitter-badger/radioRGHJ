...
\affil{$^5$Department of Mechanical and Aerospace Engineering, Princeton University, Princeton, NJ 08544}
...
\begin{abstract}
  ...
  100~MHz~$(B/3 {\rm~Gauss})$.  Such a planet might generate a radio
...
\begin{equation}
f_{\rm cyclotron} = \frac{eB}{2\pi m_e c} \approx 3 {\rm~MHz} \left( \frac{B}{1 \rm~G} \right) \, .
\label{eq:cyc}
\end{equation}
...
\end{eqnarray}
In the above, $d_A$ is the distance away from the planet where the
magnetic energy density equals the kinetic energy density in the
stellar wind, if $B_0$ is the magnetic field strength at distance
$d_0$.
