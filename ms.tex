...
In this regime, there are a number of current and near-future radio wave detectors including GMRT, LOFAR, LWA, MWA, Hera, SKA, and upgrades to the VLA. 
...
We estimate the noise in the lower half of the band ($\nu<200~\rm MHz$) assuming
$T_{\rm sys} \approx T_{\rm sky}$, and use the fit from \cite{Rogers+Bowman2008}:
\begin{equation}
...
For the rest of the instruments considered, we also assume $T_{\rm sys} \approx T_{\rm sky}$,
and adopt the \cite{Rogers+Bowman2008} relation.  We caution that is assumption is 
optimistic, and the time estimates could in reality be a factor of a few worse.
